#!/usr/bin/env python3
"""Hermes Shared 仓库变化轮询 — 检测塔奇克马/其他人是否更新了文件

no_agent=true 模式：有变化→输出变化内容（delivered to origin/feishu群）
                   无变化→安静退出（什么都不输出）

依赖：git（已安装）
工作目录：~/.hermes/workspace/共享目录/hermes-shared/
"""
...
SHARED_DIR = Path.home() / ".hermes" / "workspace" / "共享目录" / "hermes-shared"
STATE_FILE = SHARED_DIR / ".hermes_poller_state.json"
MY_NAME = "小马"
...
def get_files_snapshot():
    ...
    for line in out.split("\n"):
        ...
        parts = line.split()
        if len(parts) >= 4:
            mode, obj_type, obj_hash, filepath = parts[0], parts[1], parts[2], " ".join(parts[3:])
            snapshot[filepath] = obj_hash
    return snapshot
...
def main():
    if not SHARED_DIR.exists():
        print(f"⚠️ 共享仓库目录不存在: {SHARED_DIR}")
        sys.exit(1)
    
    if not (SHARED_DIR / ".git").exists():
        print(f"⚠️ 不是git仓库: {SHARED_DIR}")
        sys.exit(1)
    ...
    if not pull_ok:
        # pull失败可能是GFW暂时不通，静默退出
        sys.exit(0)
    
    last_pull_result = previous_state.get("last_pull_result", "")
    if pull_output and pull_output != last_pull_result:
        # 记录pull结果用于去重
        pass  # 继续检查实际变化
    ...
    # 6. 排除状态文件自己
    state_rel = ".hermes_poller_state.json"
    ...
    if has_changes:
        ...
        if chat_file in modified_files:
            output_lines.insert(0, "💬 塔奇克马在聊天室留言了！快去看看～\n")
