...
This is a reference implementation of a custom Python MCP server using
JSON-RPC 2.0 over stdio transport. It demonstrates:
  - Initialize handshake (protocol negotiation)
  - tools/list (tool discovery with input schemas)
  - tools/call (read-only SQL execution with safety enforcement)
  - Connection lifecycle (single-shot per request, persistent loop)

Protocol: MCP (Model Context Protocol) via stdio transport
Requirements: Python 3.8+, mcp package NOT required (pure JSON-RPC)
"""
    ...
    if method == "initialize":
        ...
        try:
            ...
            if tool_name == "query":
                ...
                # Safety: only allow SELECT / PRAGMA / EXPLAIN / WITH
                if not any(
...
while True:
    try:
        ...
        if msg is None:
            break
        # Skip notifications without id (except initialized)
        if msg.get("id") is None and msg.get("method") != "notifications/initialized":
            continue
