...
13. **Simple shell ops (cp, mv, rm) are faster with `--allowedTools 'Bash'` than `'Read,Write'`** — using `Read,Write` for a copy forces Claude to read the file into context then write it out (2+ turns). A single `cp` command via `Bash` completes in 1 turn. Prefer `Bash` when the task is a native shell operation, reserve `Read,Write` when Claude needs to inspect or transform content.
14. **`--max-turns 3` is too few for any real work** — even a `Read,Write` file copy needs 3-5 turns (thinking + read + write). Start at `--max-turns 5` for trivial tasks, 10-15 for real work. Running out of turns mid-task produces `Error: Reached max turns (N)` — not a timeout, but a silent cut-off.
15. **Always set `timeout >= 120` in terminal calls** — Claude Code has startup overhead (model loading, system prompt expansion). Even a trivial `cp` command may take 30-60s to boot and respond. A 30s timeout will kill the command before Claude has even started working.

## OpenAI Codex CLI Support (port of Codex skill)

Codex CLI (OpenAI's autonomous coding agent) can be delegated to in the same pattern as Claude Code, with three modes: `review`, `challenge`, and `consult`.
...
11. **When the user says "指挥" (command/direct) a sub-agent, DELEGATE** — send the task through Claude Code (or the named agent), don't do it manually. The user wants the sub-agent to execute, not you. Even for trivial ops where you could do it in one command, if they said "指挥", use the agent.
