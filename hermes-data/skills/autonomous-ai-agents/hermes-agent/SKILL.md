...
5. **Provider missing from model picker**: The model picker uses cached provider/model lists stored in `$HERMES_HOME/provider_models_cache.json`. If this cache is stale or incomplete (e.g., only showing 4 providers when 30+ exist), the picker won't display those providers' models. Fix:
   - **Non-interactive**: `hermes config set model.default <provider>/<model>` sets the model directly
   - **Interactive refresh**: `hermes model --refresh` clears the cache and re-fetches (TUI mode)
   - **Verify cache**: Check `provider_models_cache.json` has your provider listed; the full catalog lives in `models_dev_cache.json`
...
- **Telegram bot can't see group messages**: By default, `can_read_all_group_messages` is `false`. The bot only receives `/commands` and `@mentions` in groups. Fix: (1) Go to @BotFather → `/mybots` → select bot → Bot Settings → Group Privacy → Turn OFF. (2) Remove bot from the group. (3) Re-add bot to the group. Only after re-adding does the privacy change take effect. Verify with `curl -s -x "http://127.0.0.1:7897" "https://api.telegram.org/bot<TOKEN>/getMe"` — `can_read_all_group_messages` should be `true`. Group messages route to a separate session like `telegram:group:<chat_id>:<user_id>` — not visible in the DM session. Hermes `allowed_chats` being empty means DMs-only; group chats are auto-detected by the gateway.
- **WeChat (Weixin) pairing**: When `WEIXIN_DM_POLICY=pairing` (default), new users must message the bot first → appear in `hermes pairing list` → admin approves with `hermes pairing approve weixin <code>`. Key caveat: ilinkai-connected WeChat bots use openid-format IDs (e.g. `o9cq8070r0xMj4UVxbbaTsB-MBlk@im.wechat`), NOT the user's `wxid_`. You cannot pre-add users by wxid — ilinkai assigns different openids that are only revealed when the user messages the bot. To onboard a new user: (1) they must find the bot's service account name in WeChat search, (2) send a message, (3) admin approves via pairing. Sharing a contact card from an ilinkai service account is not possible — the user must search for the bot by name.
