...
~/.hermes/skills/gstack/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.hermes/skills/gstack/bin/gstack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
- `AWS access key` regex: `***AWS_PREFIX***[0-9A-Z]{16}`
- `AWS secret key` style: 40-char base64 with `aws_secret_access_key` nearby
- `GitHub token`: `***PAT_PREFIX***_[A-Za-z0-9]{36}`, `***PAT_PREFIX_O_[A-Za-z0-9]{36}`, `***PAT_PREFIX_S_[A-Za-z0-9]{36}`
