...
~/.hermes/skills/gstack/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.hermes/skills/gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
>   ✅ Zero local state — only `~/.hermes.json` MCP registration
...
resting state in `~/.hermes.json` mode 0600.
...
~10ms. The token's resting state is `~/.hermes.json` (mode 0600 — Claude
...
- Token: stored in ~/.hermes.json (do not commit; never written to AGENTS.md)
...
in to git in many projects). It lives only in `~/.hermes.json` where
