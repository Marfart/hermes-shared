...
**⚠️ Y板槽位限制（常见错误！）**：
- BL118/BL118A：只有 **1个Y板槽**，只能插1块Y板
- BL118B：只有 **2个Y板槽**，最多插2块Y板
- 客户要4种不同IO类型（DI+AI+TC+脉冲）时，2块Y板不够用，至少2种功能必须通过RS485外接Modbus RTU模块实现
- 对比：IOy系列（BL190等）有 **3个Y板槽**，但无4G/Node-RED
