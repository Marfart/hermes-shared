...
- **Windows Admin-elevated launches** — read `references/windows-admin-launch.md`
  (ShellExecuteW + "runas" pattern for tools that need admin rights,
  then CUA automation once the process is running).
...
- `cloudflare-bypass.md` — browser proxy blocks caused by Browserbase IP being flagged by Cloudflare, plus the CDP local-Chrome workaround to register on international platforms.
