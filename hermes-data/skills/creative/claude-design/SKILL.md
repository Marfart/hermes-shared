...
## Sketch Workflow (Throwaway HTML Mockups)

When the user wants to **compare 2-3 design directions side-by-side** before committing, use this lightweight variant workflow. The point is exploration, not production code.

### When to Use This (vs full claude-design)

| Use case | Skill to use |
|----------|-------------|
| Compare 2-3 visual directions quickly | **Sketch workflow** (this section) |
| Design a polished one-off artifact | claude-design (main body above) |
| Match a known brand's look | popular-web-designs + claude-design |
| Author DESIGN.md tokens | design-md |

### Intake (1-2 questions)

1. **Feel.** "What should this feel like?" — adjectives, emotions, a vibe
2. **References.** "What apps or sites capture this feel?"
3. **Core action.** "What's the single most important user action on this screen?"

### Variant Axes (pick one, pull apart)
...
Two variants differing only in accent color are wasted effort. Each must take a **different design stance**.
...
- Tailwind CDN is fine for speed.
- Realistic fake content (actual sentences, not lorem ipsum).
- **Interactive**: at least one clickable state transition or hover effect.
...
**Always verify visually** using `browser_navigate` + `browser_vision` — catches layout bugs that source inspection misses.

### Comparison Table

After all variants are built, present a head-to-head comparison:
...
**Opinionate:** declare which variant is strongest and why. Let the user pick, combine, or iterate.
...
├── NNN-stance-name/
│   ├── index.html
│   └── README.md  (design choices + trade-offs)
└── NNN-stance-name/
    ├── index.html
    └── README.md
```
