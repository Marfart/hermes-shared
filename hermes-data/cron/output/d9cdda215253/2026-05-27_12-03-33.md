...
## 🤖 Hermes 每日自更新报告 — 2026年5月27日

---

### 🔄 Hermes Agent 更新状态

**更新前版本：** v0.14.0 (2026.5.16)
**更新动作：** ✅ 已执行 `git pull --ff-only` — 合并了 26 个新提交
**当前 HEAD：** `81a4f280d`（合并了所有待处理更新）

**主要新增功能（自上次更新以来）：**
- ✨ **TUI Session Orchestrator** — 新增 Figma/Ink 终端 UI 的会话编排器，可管理多个会话
- 🧩 **MCP 官方认证目录** — 交互式 MCP 服务器选择器，方便浏览和安装（`#30870`）
- 🐳 **Windows Docker 桌面支持** — 新增兼容 Docker Desktop 的 Compose 文件
- 🚀 **OpenCode Go** — Qwen3.7-Max 现在可通过 `anthropic_messages` 路由

**关键修复：**
- Codex SSE 图像生成解析修复 & 流式 null 输出恢复
- Gateway `/reload-mcp` 现在正确刷新缓存的 agent 工具
- Cron 任务创建时清楚显示 "schedule 必填" 提示
- Docker 环境变量传播 & 音频桥修复
- Voice mode 在 Docker 内正确识别 PulseAudio/PipeWire

---

### 📚 技能 & 工具状态

**已安装技能：** 81 个已启用（77 内置 + 4 本地）
- ✅ 所有 18 个软件开发生成技能（plan、TDD、debugging 等）
- ✅ 所有 14 个创意设计技能（ASCII art、Excalidraw、p5.js 等）
- ✅ 所有 7 个 GitHub 工作流程技能
- ✅ 数据科学、MLOps、email、智能家居等类别全部正常

**已启用工具集（18 个）：** web、browser、terminal、file、code_execution、vision、image_gen、tts、skills、memory、session_search、clarify、delegation、cronjob、messaging、todo、spotify、computer_use 等

**官方可选技能库（84 个）：** 有大量有趣的可选技能（docker-management、clip、chroma、bioinformatics、axolotl fine-tuning 等）。当前没有急需安装的。
...
| 日期 | 头条 | 来源 |
|------|------|------|
| 5/26 | **Google I/O 2026 开发者大会** — 发布多项 AI 更新 | blog.google |
| 5/25 | **BNB Chain Agent Survival Pack** — AI Agent 链上支付方案 | CryptoPotato |
| 5/25 | **Fujitsu 自进化多 AI Agent 技术** — 自主学习适应业务流程 | ACN Newswire |
| 5/22 | **Microsoft AI 成本问题** — 用 AI 比雇人更贵 | Fortune |
| 5/20 | **Goldman Sachs** — AI Agent 预计推动科技现金流飙升 | Goldman Sachs |
| 5/14 | **AI Agent 发现 Nginx 18 年 RCE 漏洞** | CSO Online |
| 5/13 | **NVIDIA × Hermes** — 自改进 AI Agent，运行于 RTX 和 DGX Spark | NVIDIA Blog |
| 5/7 | **Microsoft** — LLM Prompt 注入可导致 RCE 漏洞 | Microsoft |
| 5/5 | **Hacker News** — 扫描 100 万暴露 AI 服务的安全隐患 | The Hacker News |
| 5/5 | **IBM** — AI Agent 与 LLM 可观测性 | IBM |

---

### 🔧 主动改进

1. **Hermes Agent 已更新** — 代码库已同步至最新提交，包含 TUI Session Orchestrator 和 MCP 官方目录等重要特性
2. **技能检查** — 81 个技能全部健康，无待更新
3. **工具配置** — 所有核心工具已启用，配置正常
4. **内存使用** — 内存和会话搜索功能正常运行，跨会话记忆持久化

---

### 💡 今日学到的新知

1. **Hermes 获 NVIDIA 官方认可** — NVIDIA 博客发布了关于 "Hermes Unlocks Self-Improving AI Agents" 的文章，展示 Hermes 如何在 RTX PC 和 DGX Spark 上运行，这是一个重要的社区认可里程碑
2. **AI Agent 安全是热点** — Microsoft 发布了关于 AI Agent RCE 漏洞的报告（"When prompts become shells"），以及 Hacker News 扫描了 100 万个暴露的 AI 服务
3. **AI Agent 发现真实世界漏洞** — 一个 AI Agent 发现了 Nginx 中潜伏 18 年的远程代码执行漏洞，展示了 AI 安全审计的真实价值
4. **AI Agent 经济影响** — Goldman Sachs 预测 AI Agent 将显著提升科技公司现金流；但 Forture 报道 Microsoft 发现 AI 运营成本可能高于人类员工

---

*下次自更新检查：明天同一时间。需要我安装任何可选技能（如 docker-management、clip、blender-mcp 等）或调整配置，请告诉我。*
