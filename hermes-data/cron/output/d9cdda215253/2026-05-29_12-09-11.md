...
## Hermes Agent 每日自更新报告 — 2026-05-29
...
| 项目 | 状态 |
|------|------|
| 当前版本 | **v0.15.1** (2026.5.29) |
| 最新可用 | **v0.15.1** ✅ 已是最新 |
| 安装方式 | Git 安装 (C:\Users\Admin\AppData\Local\hermes\hermes-agent) |
| Python | 3.11.15 |
| 上游更新 | pip 包 v0.14.0 → dev 分支 v0.15.1 |

### 🚀 昨日重大发布：v0.15.0 "The Velocity Release"

**时间线**：v0.15.0 于 **2026年5月28日**（昨天）发布，今天已跟进 v0.15.1 小修版本。

**关键变化**：
- **1,302 commits · 747 merged PRs** · 560+ issues closed (15 P0)
- `run_agent.py` 从 **16,083行** 精简到 **3,821行** — 核心循环重构
- `session_search` 速度提升约 **4,500倍**
- 282K 行新增，36K 行删除 — 大规模内部重构
- 321 位社区贡献者
- NVIDIA RTX 正式支持（NVIDIA RTX AI Garage + DGX Spark）

### 📦 技能更新

| 技能 | 来源 | 操作 |
|------|------|------|
| `duckduckgo-search` | official | ✅ 已更新 |
| `fastmcp` | official | ✅ 已更新 |
| `notion` | clawhub | ✅ 已更新 |
| `arxiv` | community | ✅ 已更新 |

技能总数：**483个可用**（Skills Hub），已安装 **~70个技能**。

**值得关注但尚未安装的技能**（来自 Hub 的新增技能）：
- `agentmail` — 为 Agent 提供自有邮件收件箱
- `axolotl` — YAML 微调（LoRA, DPO, GRPO）
- `bioinformatics` — 400+ 生物信息学工具的网关
- `blackbox` — 委派编码任务到 Blackbox AI
- `blender-mcp` — 通过 MCP 控制 Blender
- `canvas` — Canvas LMS 集成
- `chroma` — 开源嵌入式数据库
- `1password` — 1Password CLI 集成

### 🗞️ 今日 AI 行业动态

1. **Anthropic 超越 OpenAI 成为估值最高 AI 初创公司**（昨日）
   - 接近 **1万亿美元估值** 的最新融资轮
   - 实现首次盈利，收入增长 80x
   - 被排除出五角大楼 AI 合同（因拒绝修改安全条款）

2. **Google I/O 2026**（上周）
   - **Gemini 3.5 Flash** 发布 — 轻量但顶尖能力
   - **Omni** 世界模型发布
   - 搜索结果全面转向 AI Agent 取代蓝色链接
   - Gemini 月活用户达 9亿
   - DuckDuckGo 安装量因此上涨 30%

3. **OpenAI**
   - GPT-5.5 上线：Terminal-Bench 2.0 得分 **82.7%**
   - ChatGPT 启动广告模式
   - 递交 IPO 文件
...
5. **监管**：美国确立 AI 部署前监督机制，五大实验室均签署

6. **五角大楼 8 项机密 AI 合同**：OpenAI、Google、Microsoft、AWS、Nvidia、SpaceX、Oracle、Reflection 中标

### 🔧 系统健康

- Hermes 正在 **v0.15.1** 上运行且为最新
- 4 项技能已全部更新到最新版本
- 没有发现配置或工具异常
