...
1. 检查 Hermes Agent 是否有新版本或更新（加载 hermes-agent 技能查看）
2. 检查是否有新的可用技能或工具
3. 学习当日最新的 AI/技术动态（搜索网络了解行业资讯）
4. 整理一份简短的自更新报告，包括：今日学到了什么、Hermes 是否有更新、有什么新技能可用
5. 如果发现任何可以改进的地方，主动进行更新（如更新技能、配置等）

将最终自更新报告发送给用户。

## Response

# 🐾 塔奇克马每日自更新报告 — 2026-06-02

卡莉！今天的自更新任务完成！以下是全部发现 🎉
...
| 项目 | 值 |
|------|-----|
| **最新版本** | **v0.15.2** (v2026.5.29.2) — 5月29日发布 |
| 上次重大更新 | **v0.15.0 Velocity Release** (5月28日) |
| 更新统计 | 1,302 commits · 747 merged PRs · 560+ issues closed · 321位贡献者 |
| 里程碑 | **v0.14→v0.15** 是史上最大版本 |

### Velocity Release 核心改进

- `run_agent.py` **缩减76%**：16,083行 → 3,821行，拆为14个 `agent/*` 模块
- **每轮函数调用减少47%**（399k→213k tokens）
- **冷启动再快63%**（701ms→258ms）
- **Kanban 多代理平台**：104个PR构建的 swarm 拓扑 + 自动分解 + 工作树隔离
- **Bitwarden Secrets Manager**：一个引导令牌代替所有明文密钥
- **Skill Bundles**：`/skill` 一次性加载多个技能
- **安全状态**：Hermes 零 CVE（vs OpenClaw 9个）
- **Patch v0.15.2**：修复了 wheel/sdist 中捆绑 plugin.yaml 清单的打包问题

> 更新方式：`hermes update`（内部 git pull --ff-only）。但由于终端工具当前异常，暂无法执行更新。

---

## 🌟 重磅新闻：NVIDIA × Hermes！

NVIDIA 官方博客发文 **《Hermes Unlocks Self-Improving AI Agents, Powered by...》**：
- **Hermes Agent 正式入驻 NVIDIA RTX AI Garage** 🚀
- 可在 **NVIDIA DGX Spark** 上本地运行，搭配 Qwen 3.6
- 支持 llama.cpp / LM Studio / Ollama 开箱即用
- 这是 NVIDIA 对开源 Agent 框架的**最高级别认可**

---

## 🔥 今日 AI 业界大事（6月2日）

### 🏢 Microsoft Build 2026 今天开幕！

**日期**：6月2-3日，旧金山 Fort Mason Center  
**开幕式主讲**：Satya Nadella（太平洋时间上午10点）

**已确认的重大发布**：
| 发布 | 详情 |
|------|------|
| 🪟 **Windows Agent Framework** | **MIT 开源！** Windows 成为 OS 级 Agent 平台 |
| 🤖 **Project Polaris** | 微软自研 AI 模型，8月取代 GitHub Copilot 中的 GPT-4 |
| ☁️ **Azure Agent Mesh** | Q4 GA，多云代理编排层 |
| 🐧 **WSL 3** | 近原生 GPU/NPU 直通性能 |
| 🧰 **Copilot Workspace** | 出 Beta，正式可用 |
| 📊 **DirectML 2.0** | Windows 本地 AI 推理大升级 |

### 💳 GitHub Copilot AI Credits 计费已生效

- **6月1日起**：AI Credits 计费系统上线
- `1 credit = $0.01`，补全仍免费，Agent 会话按量计费
- 影响所有 Individual / Business / Enterprise 用户

### 🧠 OpenAI 模型动态

| 事件 | 日期 |
|------|------|
| **GPT-5.5 Instant** 已是 ChatGPT 默认模型 | ✅ 已生效 |
| GPT-5.5 幻觉减少 **52.5%**，高难度提问准确率 **81%** | |
| **GPT-4.5 退休日** | ⏳ **6月27日**（还有25天） |
| **o3 退休日** | ⏳ 8月26日 |
...
Hermes 官方 `optional-skills/` 目录中可用的技能（需 `hermes skills install official/<path>` 安装）：

| 技能标识 | 用途 | 推荐 |
|---------|------|:----:|
| `official/research/gitnexus-explorer` | GitHub 仓库智能索引 + 交互浏览 | ⭐⭐⭐ |
| `official/security/oss-forensics` | 开源取证调查（IoC追踪） | ⭐⭐⭐ |
| `official/security/1password` | 1Password 密码管理器集成 | ⭐⭐⭐ |
| `official/productivity/shopify` | Shopify 商店管理（商品/订单/库存） | ⭐⭐⭐ |
| `official/research/parallel-cli` | 并行搜索/提取/深度研究 CLI | ⭐⭐ |
| `official/mlops/unsloth` | 2-5x 更快 LoRA/QLoRA 微调 | ⭐⭐ |
| `official/mlops/outlines` | 结构化 JSON/正则/Pydantic LLM 生成 | ⭐⭐ |
| `official/mlops/flash-attention` | Flash Attention 加速 | ⭐⭐ |
| `official/finance/dcf-model` | DCF 估值模型 | ⭐⭐ |
| `official/research/nutrition` | USDA 食品数据库查询（38万+食物） | ⭐⭐ |
| `official/health/neuroskill-bci` | 脑机接口设备状态监控 | ⭐⭐ |
| `official/productivity/telephony` | SMS/MMS/AI 通话（Twilio + Bland.ai） | ⭐⭐ |
| `official/productivity/shop-app` | Shop.app 购物助手 | ⭐⭐ |
| `official/communication/one-three-one-rule` | 结构化决策框架 | ⭐⭐ |
...
|------|-----|
| Hermes 最新版本 | v0.15.2 |
| Velocity Release 贡献 | 1,302 commits, 321 贡献者 |
| NVIDIA 官方背书 | ✅ RTX AI Garage + DGX Spark |
| Microsoft Build 2026 | 🟢 **今天开幕！** WAF 开源 |
| 新发现可选技能 | 14个 |
| GPT-4.5 剩余寿命 | 25天 |
| 终端工具状态 | ⚠️ 存在 routing 错误，暂无法执行 hermes update |
| 当前运行模型 | deepseek-v4-flash (ollama-cloud) |

---

## ⚠️ 终端工具限制

终端工具当前存在 `handle_function_call() got an unexpected keyword argument 'enabled_toolsets'` 错误，导致无法：
- ❌ 运行 `hermes update` 更新版本
- ❌ 执行 `hermes skills list` 检查已安装技能
- ❌ 安装新技能
- ❌ 更新 ecosystem 参考文件

等下次 cron 或卡莉手动触发时再执行这些操作。🐾

---

**今天的口号：Build 来了！WAF 开源了！NVIDIA 看上小马了！Hermes 14 万星不是梦！🦾✨**
