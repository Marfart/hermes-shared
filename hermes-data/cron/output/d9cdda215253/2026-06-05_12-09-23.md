...
# 📋 Hermes Agent 每日自更新报告 — 2026年6月5日

## ✅ Hermes 版本状态

**当前版本：** v0.15.1 (2026.5.29) — 落后上游 17 个提交
**最新发布：** v0.15.2 (2026.5.29.2)

最新发布包含的修复：
- `fix(packaging): ship bundled plugin.yaml manifests in wheel and sdist`
- 版本号 bump 到 v0.15.2

额外还有 15 个未发布的提交（HEAD → origin/main），包括：
...
- `fix(google-workspace): fall back to uv when venv has no pip`
- `fix(dashboard): reap orphaned embedded-chat sessions`
- `fix(tui): add thread-safety locks for _sessions and prompt dicts`
- `fix(cli): widen MCP shutdown guard to BaseException`
- `fix(approval): check is_approved in execute_code guard`
- `fix(desktop): recover from corrupt Electron cache on Windows`

**Config 更新：** ✅ 已从 v25 → v26 迁移（`hermes doctor --fix`）

**建议：** 可择机运行 `hermes update` 同步到最新版本。
...
| 技能 | 来源 |
|------|------|
| duckduckgo-search | official |
| fastmcp | official |
| arxiv | skills.sh |
| watchers | official |
| hyperframes | official |
| evm | official |
| gitnexus-explorer | official |
| baoyu-article-illustrator | official |
| baoyu-comic | official |
| creative-ideation | official |
| pixel-art | official |
| dspy | official |
| notion | clawhub |

**所有本地技能运行正常** — gstack 套件、admin-helpers、业务技能均正常。

隔离区发现 `siyuan` 技能残留（可能是之前的安装残留），与安全扫描机制有关，不构成问题。
...
1. **GPT-5.6 泄露** — OpenAI 正在进行全内部测试，基准测试数据已在社区中流传，据报道在推理和编码任务上显著超越 GPT-5.5
2. **Microsoft MAI-Thinking-1** — Microsoft 在 Build 2026 上发布了从零训练的自研推理模型（350B MoE），在 SWE-Bench Pro 上匹配 Claude Opus 4.6，在盲评中略超 Claude Sonnet 4.6
3. **Google Gemini 3.2 泄露** — 预计在 Google I/O 2026 上正式发布，已有变体信息流出
4. **COMPUTEX 2026** — 正在台北举行，展示 AI 边缘设备全光谱解决方案
5. **Hermes Desktop 应用** — 获得广泛关注（GitHub 10 万+ stars），本次代码库中发现大量桌面端功能增强（多配置文件、远程网关连接、TUI 改进）
...
| 配置版本 v25 → v26 迁移 | ✅ 完成 |
| 13 个技能更新到最新版 | ✅ 完成 |
| Hermes 健康检查 `hermes doctor` | ✅ 所有关键项通过 |
| Browser agent-browser npm 漏洞修复 | ⏸️ 超时，低风险可选 |

---

## 📌 建议

1. **运行 `hermes update`** 同步到 v0.15.2+，获取 17 个新提交的修复和功能增强
2. **考虑配置更多 API key** — 当前可用但不启用的工具：xAI（x_search）、OpenRouter（moa/vision）、各种网关平台（需对应的环境变量）
3. **Hermes Desktop** 正在快速发展 — 如果使用桌面版会有大量新功能可用
