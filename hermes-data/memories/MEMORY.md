...
§
文件发送规则更新（2026-06-12）：PDF用tmpfiles.org上传（curl→URL→验证Content-Type→发/dl/链接）。HTML日报唯一可行分享方案是Cloudflare Tunnel（cf.exe在Desktop/Working/，python -m http.server 8899 → cf.exe tunnel → trycloudflare.com链接）。catbox/litterbox/0x0/file.io/transfer.sh/pixeldrain/gofile/GitHub Gist对HTML全部不可用。不发MEDIA:path不发本机路径。
§
