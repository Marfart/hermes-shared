...
async function main() {
  ...
  for (let page = 1; page <= 40; page++) {
    ...
    for (const row of values) {
      ...
      if (n.includes("hsinsight") || n.includes("hussain")) {
        console.log(`Found: ID=${row.id}, Name=${row.name}`);
        ...
        const r = await client.addFollowRecord({
          customerId: row.id,
          content: "他之前说想下单一个，问多久能到货。公司名HSInsight，给了邮箱Hussain@hsinsight.io和电话。我回复说收到付款了会尽快处理",
          planningTime: ts,
