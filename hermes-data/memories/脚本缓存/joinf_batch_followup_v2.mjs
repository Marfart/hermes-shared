...
// 今天聊过的客户列表（跳过已处理的）
const CUSTOMERS = [
  { name: "LA", company: "LA", followup: "他回复说理解电池不包含在标准发货中，我跟他确认了这件事" },
  { name: "Subhodeep", company: "Subhodeep", followup: "他说定制项目的申请流程需要一些时间，等他们领导层审核完我的具体要求后会给我报价，我回复说好的没问题" },
  { name: "Abhinav", company: "Abhinav", followup: "他说这两款入门级产品卖得很好，我跟他确认了产品情况" },
  { name: "Вадим", company: "Вадим", followup: "他说在等我的回复，我回复了感谢" },
  { name: "Areej Procurement", company: "Areej Procurement", followup: "他问我要DHL追踪号，我解释说只有发货后才能拿到追踪号，让他先确认具体需求" },
  ...
  { name: "Bikramjit Singh", company: "Bikramjit Singh", followup: "他催我检查货物并安排税款支付，说海关10号就放行了但我这边还没交税，对方在催他" },
  { name: "Dudley May", company: "Dudley May", followup: "他说如果价格有竞争力且工业性能稳定，哥伦比亚市场有很好的商业机会引入我们的方案" },
  { name: "SYED ALI", company: "SYED ALI", followup: "我跟他解释了最近市场波动大，某些SOM板价格涨了不少，如果觉得价格高可以考虑其他型号" },
  { name: "Enrique Madrigal", company: "Enrique Madrigal", followup: "我跟他解释了SOM452生产成本大幅上涨，单价调整到$781.53，如果接受不了可以看其他替代型号" },
  ...
  { name: "Shafqat Paracha", company: "Shafqat Paracha", followup: "等待加载消息" },
...
async function main() {
  ...
  for (const c of CUSTOMERS) {
    ...
    if (result.matched) {
      ...
      if (r.json?.success) {
        added++;
        console.log(`✅ ${c.name} → ${result.customerName} (ID: ${result.customerId}) ✓`);
      } else {
