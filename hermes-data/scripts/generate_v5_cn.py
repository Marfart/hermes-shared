...
# -*- coding: utf-8 -*-
"""v5-CN - Chinese version of the ZODSAT project spec document"""
...
rl2 = pf2.add_run(" 页")
rl2.font.size = Pt(7.5); rl2.font.color.rgb = RGBColor.from_string("94A3B8")
...
mk_table(doc, ["功能", "说明"], [
    ...
    ["物联网锁（新增）", "电子柜锁（含隐藏机械应急钥匙）。集成 ThingsBoard 平台，支持远程开锁、操作人员记录及维护审计追踪。"],
])
...
body_txt(doc, "说明：平台部分独立于 S275 硬件。此处仅涉及 payload 格式、数据字段、告警规则及集成接口参考，供技术团队提前知悉。", sz=8.5, clr="64748B", it=True)
...
body_txt(doc, "*外购配件清单：11.1V / 5Ah 锂电池、100W MPPT 太阳能充电器、物联网柜锁（隐藏应急钥匙）、8dBi 天线+3米馈线、杆装支架。", sz=7.5, clr="64748B", it=True, sb=2, sa=1)
body_txt(doc, "50W 促销优惠价：2,000 台 @ $149.00  |  5,000 台 @ $144.00  |  10,000 台 @ $139.00", sz=8, clr="1D4ED8", b=True)
...
body_txt(doc, "报价术语：EXW（深圳）。所有价格以美元计。", sz=8, clr="64748B", it=True, sb=2, sa=0)

# ====== SECTION 7 ======
sec_title(doc, "七", "项目开发与交付时间表")
body_txt(doc, "初步排期 — 以规格冻结及定金支付后确认为准。", sz=8, clr="B91C1C", it=True)
...
cll(sr.cells[0], "公司公章（如适用）", sz=8.5, clr="64748B")
