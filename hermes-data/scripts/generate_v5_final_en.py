...
# -*- coding: utf-8 -*-
"""v5.1-FINAL - Polished professional document matching reference style"""
...
info = [
    ...
    ("Version", "V5.1 — Final Specification"),
    ("Date", "27 May 2026"),
...
mk_table(doc, ["Module", "Specification"], [
    ...
    ["Data Path", "Node → Milesight SG50 Gateway → App Server → MQTT Cloud Platform."],
    ...
    ["External Antenna", "8 dBi fibre-glass antenna with 3 m cable."],
...
mk_table(doc, ["Function", "Description"], [
    ...
    ["IoT Cabinet Lock", "Electronic cabinet lock with hidden backup key. Integrates with ThingsBoard for remote unlock, operator access logging, and maintenance audit trail."],
])
...
mk_table(doc, ["Item", "Specification"], [
    ...
    ["Installation Options", "(1) Outdoor/pole: panel mounted on TAIS unit with pole-mount stand.\n(2) Indoor/wall: panel on roof, cabled to unit."],
...
body_txt(doc, "Note: Platform scope is separate from S275 hardware. Included for payload format, data fields, alarm rules, and integration interface reference.", sz=6.5, clr="B91C1C", it=True)
mk_table(doc, ["Item", "Requirement"], [
    ...
    ["Front-end & Backend", "Full UI + backend development to replace Akenza API."],
...
body_txt(doc, "*Accessories: 11.1 V / 5 Ah Li-ion Battery, 100 W MPPT Solar Charger, IoT Cabinet Lock (hidden key override), 8 dBi Antenna + 3 m Cable, Pole-Mount Stand.", sz=6, clr="B91C1C", it=True, sb=2, sa=1)
body_txt(doc, "Promotional (50 W): 2,000 @ $149.00  |  5,000 @ $144.00  |  10,000 @ $139.00", sz=7.5, clr="1F4E79", b=True)
...
pt_data = [
    ("Deposit", "50% of total order value upon order confirmation."),
...
body_txt(doc, "Quotation Term: EXW (Shenzhen, China)  •  All prices in USD.", sz=6.5, clr="B91C1C", it=True, sb=1, sa=0)

# ====== SECTION 7 ======
sec_title(doc, "7", "Project Development & Delivery Schedule")
body_txt(doc, "Preliminary — subject to confirmation upon specification freeze and deposit.", sz=6.5, clr="B91C1C", it=True)
...
cll(sr.cells[0], "Company Stamp / Seal (if applicable)", sz=8, clr="64748B")
