...
# -*- coding: utf-8 -*-
"""v4 - Transparent tables so watermark shows through, with border-based styling"""
...
def mk_table(doc, hdrs, rows, hdr_width_pct=None):
    """Create a table with NO cell shading — watermark shows through.
       Professional look via borders and text formatting only."""
    t = doc.add_table(rows=1+len(rows), cols=len(hdrs))
    t.alignment = WD_TABLE_ALIGNMENT.CENTER; tbl_borders(t)
    for i, h in enumerate(hdrs):
        c = t.rows[0].cells[i]
        mg(c, 80, 80, 150, 150)
        # Header: bold dark text with bottom border (no background fill)
    ...
    for ri, rd in enumerate(rows):
        for ci, tx in enumerate(rd):
            ...
            mg(c, 60, 60, 150, 150)
            # Alternating row: slight variation via text color only
...
def info_box(doc, txt, sz=9, clr="1E3A5F"):
    """Info box with border only — no background fill, watermark shows through."""
    t = doc.add_table(rows=1, cols=1); t.alignment = WD_TABLE_ALIGNMENT.CENTER
    c = t.rows[0].cells[0]
    mg(c, 100, 100, 180, 180)
    # Uses border with blue tint, no fill
...
def dark_banner(doc, line1, line2):
    """Banner with no fill — uses thick borders and text emphasis instead."""
    t = doc.add_table(rows=1, cols=1); t.alignment = WD_TABLE_ALIGNMENT.CENTER
    c = t.rows[0].cells[0]
    mg(c, 100, 100, 180, 180)
    # Add thick bottom border + blue left accent instead of background fill
    tcPr = c._tc.get_or_add_tcPr()
    borders = ET.SubElement(tcPr, qn('w:tcBorders'))
