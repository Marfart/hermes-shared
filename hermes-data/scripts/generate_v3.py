...
# -*- coding: utf-8 -*-
"""v3 - Professional doc with transparent watermark"""
...
# ---- HELPERS ----
def shd(c, clr):
    c._tc.get_or_add_tcPr().append(parse_xml('<w:shd %s w:fill="%s"/>' % (nsdecls("w"), clr)))
...
def mk_table(doc, hdrs, rows):
    ...
    for i, h in enumerate(hdrs):
        c = t.rows[0].cells[i]; shd(c, "1E3A5F"); mg(c, 100, 100, 150, 150)
        cll(c, h, b=True, sz=9, clr="FFFFFF")
    for ri, rd in enumerate(rows):
        for ci, tx in enumerate(rd):
            c = t.rows[ri+1].cells[ci]
            shd(c, "F8FAFC" if ri%2==0 else "FFFFFF"); mg(c, 70, 70, 150, 150)
            cll(c, str(tx), b=(ci==0), sz=8.5, clr="1E293B" if ci==0 else "475569")
...
def info_box(doc, txt, sz=9, clr="1E3A5F"):
    ...
    c = t.rows[0].cells[0]; shd(c, "EFF6FF"); mg(c, 120, 120, 180, 180)
    tbl_borders(t, "BFDBFE", "BFDBFE", "BFDBFE", "BFDBFE", "BFDBFE")
...
def dark_banner(doc, line1, line2):
    t = doc.add_table(rows=1, cols=1); t.alignment = WD_TABLE_ALIGNMENT.CENTER
    c = t.rows[0].cells[0]; shd(c, "1E3A5F"); mg(c, 100, 100, 180, 180)
    tbl_borders(t, "1E3A5F", "1E3A5F", "1E3A5F", "1E3A5F", "1E3A5F")
    cll(c, line1, b=True, sz=12, clr="FFFFFF", al=WD_ALIGN_PARAGRAPH.CENTER)
    cll(c, line2, sz=8, clr="93C5FD", al=WD_ALIGN_PARAGRAPH.CENTER)
...
info = [
    ...
    ("Version", "V3.0 — Document Control: Updated Specification"),
    ("Date", "27 May 2026"),
    ("Classification", "Confidential — For Authorized ZODSAT Personnel Only"),
]
it = doc.add_table(rows=len(info), cols=2); it.alignment = WD_TABLE_ALIGNMENT.CENTER
tbl_borders(it)
for i, (l, v) in enumerate(info):
    bg = "F8FAFC" if i%2==0 else "FFFFFF"
    cl = it.rows[i].cells[0]; shd(cl, bg); mg(cl, 50, 50, 120, 120)
    cll(cl, l, b=True, sz=9, clr="1E293B")
    cv = it.rows[i].cells[1]; shd(cv, "FFFFFF" if i%2==0 else "F8FAFC")
    mg(cv, 50, 50, 120, 120); cll(cv, v, sz=9, clr="475569")
...
mk_table(doc, ["Module", "Specification"], [
    ...
    ["External Antenna", "7 dBi fibre-glass antenna with 3 m cable."],
...
mk_table(doc, ["Function", "Description"], [
    ...
    ["PT100 Temperature", "Oil and winding temperature monitoring."],
...
mk_table(doc, ["Item", "Specification"], [
    ...
    ["Battery Type", "12 V / 5 Ah rechargeable Li-ion with BMS (overcharge/discharge protection)."],
    ...
    ["Installation Options", "(1) Outdoor/pole: panel on TAIS unit. (2) Indoor/wall: panel on roof, cabled."],
...
mk_table(doc, ["Item", "Requirement"], [
    ...
    ["Integration Boundary", "Scope, timeline, cost defined separately. Hardware provides payload definitions and data fields."],
])
...
body_txt(doc, "*Accessories: 12 V / 5 Ah Li-ion Battery, 100 W MPPT Charger, IoT Cabinet Lock (hidden key), 8 dBi Antenna + 3 m Cable, Pole-Mount Stand.", sz=7.5, clr="64748B", it=True, sb=2, sa=1)
...
for j, h in enumerate(["Field", "Information to be Completed"]):
    c = st.rows[0].cells[j]; shd(c, "1E3A5F"); mg(c, 80, 80, 120, 120)
    cll(c, h, b=True, sz=9, clr="FFFFFF")
...
for i, lbl in enumerate(sig_data):
    ...
    bg = "F8FAFC" if i%2==0 else "FFFFFF"
    shd(cl, bg); mg(cl, 60, 60, 120, 120); cll(cl, lbl, b=True, sz=9, clr="1E293B")
    mg(cv, 60, 60, 120, 120); cll(cv, "_"*50, sz=7, clr="CBD5E1")

sr = st.rows[6]; sr.cells[0].merge(sr.cells[1])
shd(sr.cells[0], "FAFAFA"); mg(sr.cells[0], 60, 60, 120, 120)
